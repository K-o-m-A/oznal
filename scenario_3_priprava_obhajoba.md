...
Na rovnakom 80/20 splite ako Scenár 2 porovnáme:

- plný Lexical GLM (13 príznakov),
- stepwise,
- lasso,
- elastic-net.

**Akceptačné kritérium**: aspoň jedna FS metóda má `k < 13` a jej held-out
AUC/Sensitivity/Specificity ostávajú prakticky použiteľné voči plnému lexical
fit-u. Nejde o produkčný claim, ale o interný benchmark na rovnakom datasete.

**Aktuálny výsledok:** stepwise a lasso zredukovali Lexical pool na 9 príznakov,
elastic-net na 11. Popri AUC reportujeme aj Sensitivity a Specificity pri prahu
0.5, presne kvôli otázke kolegu: proxy potrebuje vedieť nielen ranking, ale aj
koľko phishingov zachytí a koľko legit stránok nechá prejsť.
...
## 4. Per-fold loop — implementačné detaily

Každý fold beží:

1. **stepwise**: štart z `glm(label ~ 1)`, scope `~ .` na Lexical pre primárny
   fit; FullLite scope sa používa len vo fallback benchmarku. AIC pravidlo,
   `keep` callback ukladá p-hodnoty.
...
Každá metóda dostáva **rovnakých ~21 600 riadkov** (in-fold tréning ≈ 90% z 24k
train). Pairovanie je striktné.

### Časová náročnosť (pozorovaná)

- `stepAIC` na 24k × 34 je hlavný bottleneck. Bidirekčný robí v každom kroku
...
**V stepwise trace-i**: hranicu 0.05 počas AIC cesty prešli najmä
`NoOfOtherSpecialCharsInURL`, `NoOfSubDomain` a `CharContinuationRate`.
Prvé dve sa neskôr z aktívneho setu aj vyhodili. Toto pekne sedí na EDA
kolinearitu v URL-count clustri: keď sa pridajú silnejšie URL-length/URL-count
premenné, slabší korelovaný prediktor stratí samostatný prínos.
...
stránok necháme prejsť. Pri FullLite benchmarku sú tieto hodnoty skoro
