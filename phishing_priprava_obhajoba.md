...
## 4a. Heatmapa označených dát — Task 4 (vizualizácia dát)

### Čo heatmap je a čo NIE je

Heatmap v §4.6 je **obraz dát**, nie obraz modelu. Farebnou mapou
hodnôt zobrazíme 500 riadkov (250 phishing + 250 legit) a 10
najdiskriminatívnejších features. Ak sú triedy geometricky oddelené,
uvidíme to ako **dva farebné pásy** po tom, ako hierarchické
zhlukovanie zoradí riadky. Je to najjednoduchšie vizuálne overenie,
že trieda **vôbec sedí na geometrii features** — bez trénovania
akéhokoľvek modelu.

**Dôležité rozlíšenie pre Task 4:** heatmap sama o sebe
**nevizualizuje model**. Na to slúži surrogate tree vo `scenario_2.rmd §7`.
Heatmap je:

- **Spodný benchmark** = "koľko sa dá vidieť bez supervízie".
- **Companion** k surrogate tree = "takto vyzerajú dáta, takto RF
  o nich rozmýšľa".

Medzera medzi purity heatmapy a AUC modelu (RF ~0.97 na Lexical) je
priama miera **hodnoty supervízie** na našom datasete.

### Čo sme ladili a prečo

Tri skupiny parametrov, všetky eksplicitne:

| Knob | Hodnoty | Čo robí |
|------|---------|---------|
| Sample size | 500 riadkov (stratifikovaný) | Kompromis medzi čitateľnosťou (nie šum) a reprezentatívnosťou (nie pár outlierov). 24k by bolo nečitateľné, 50 nedostatočné. |
| Feature subset | Top-10 z §4.2.1 (|SMD| + Cramér's V) | Všetkých 40 stĺpcov by utopilo signál v slabých predictoroch; top-3 by vnucovali záver. Desiatka je prirodzený zlom kde effect-size krivka klesá. |
| Distance | euclidean vs. `1-|cor|` | Euclidean meria **absolútne rozdiely hodnôt**. Correlation meria **tvar profilu** cez features (nezáleží na škálach). |
| Linkage | ward.D2 / complete / average | Rôzne kompromisy medzi kompaktnosťou klastrov (ward) a robustnosťou voči outlierom (average). |
| k (počet klastrov) | fixne 2 | Úloha je binárna, takže požadujeme 2 klastre. Iné by bola iná otázka. |

Evalujeme všetkých 6 kombinácií (distance × linkage) a zoberieme tú s
najvyšším **adjusted Rand indexom (ARI)** voči pravde.

### Ako meriame "fidelity" heatmapy

Heatmap nemá predikcie, takže fidelity sa nedefinuje rovnako ako pri
stromoch. Namiesto toho:

- **Purity per klaster:** pre každý z 2 klastrov pozrieme, ktorá
  trieda v ňom prevláda (`max(table) / sum(table)`). Priemerná
  majority-class purity = podiel riadkov, ktoré hlasovanie "podľa
  klastra" priradilo správne.
- **Adjusted Rand index (ARI):** meria zhodu dvoch partícií (clustering
  vs. ground-truth label) korigovanú na náhodnú zhodu. ARI = 0 pri
  náhode, ARI = 1 pri dokonalej zhode.

Typický výsledok na našom datasete: ARI 0.3-0.6, purity 0.80-0.90.
**Model (RF) dosahuje AUC 0.97 na Lexical** = supervízia pridáva
podstatnú hodnotu nad tým, čo sa dá vyčítať heatmapou.

### Podobnosti a rozdiely vizualizácie vs. modelu

**Podobnosti:**
- Oba používajú tie isté features ako vstup.
- Oba odhalia, že Lexical je ťažšia úloha ako FullLite.

**Rozdiely:**
- Heatmap **nepozná label počas výpočtu** (zhlukovanie je
  unsupervised); jediné čo sa mení voči labelu je poradie v ARI
  tabuľke post-hoc.
- Heatmap je 500 × 10 → akákoľvek **interakcia medzi 3+ features**
  (typický signál phishingu) je neviditeľná.
- Model (RF, surrogate tree) trénuje na 24 000 riadkoch a všetkých
  features daného tieru, s plnou supervíziou.

### Kľúčové otázky komisie

**Otázka: Prečo iba 500 riadkov a top-10 features?**
Čitateľnosť. Pri 24k riadkoch by bola heatmap pás pixelov bez
štruktúry; pri 40 features by slabé predictory (obrátené poradie
discriminative power) utopili silné. 500 × 10 je natural zoom, ktorý
zachová štatistickú stabilitu (stratifikácia drží 50/50 class balance)
a vizuálnu čitateľnosť.

**Otázka: Čo keby ste použili iné features?**
Sanity check: random 10 features by dalo ARI blízke 0 a purity ~0.55
(náhoda). To by potvrdilo, že discriminative-power ranking v §4.2.1
je správny a že heatmap efektívne reaguje na kvalitu features, ktoré
jej dáme. (Voliteľne sa to dá reprodukovať jednou extra chunkou.)

**Otázka: Prečo nie k > 2 klastre?**
Lebo úloha je binárna. k = 3 by sa nedalo porovnať s pravdou bez
ad-hoc mapovania "ktoré 2 z 3 klastrov sú phishing". k = 2 je jediná
čistá voľba pre ARI a purity.

**Otázka: Prečo ARI a nie napríklad normalised mutual information?**
ARI je korigovaný na náhodu — pri náhodnom zhlukovaní je očakávaná
hodnota ~0. NMI nie je (pri nevyváženej kardinalite klastrov môže byť
ARI tesne pod nulou, NMI nie). Obe metriky by dali podobný ranking,
ale ARI je ľahšie interpretovateľný (0 = náhoda, 1 = perfektná zhoda).

**Otázka: Prečo heatmap, keď máte aj surrogate tree?**
Lebo ukazujú **rôzne veci**. Heatmap je "čo sa dá vidieť bez
trénovania" — slúži ako baseline pre čitateľa. Surrogate tree je
"čo náš model reálne robí" — slúži ako vizualizácia trénovaného
rozhodnutia. Dohromady odpovedajú na obe časti Task 4: *"visualize
labelled data"* (heatmap) + *"capture model-based classification"*
(strom).

**Otázka: Aký je vzťah tejto heatmapy k H1/H2 hypotézam?**
Žiadny priamy — toto je Task 4, nie H1/H2. Heatmap je **samostatný
Task 4 companion**, nie evidencia pre H1. H1 je testovaná tvrdými
číslami v `scenario_2.rmd §6.2` (paired Wilcoxon na AUC). Heatmap je
ilustratívna a necháva v §4.6 aby sme neprelievali žánre (Task 4
samostatne, H1/H2 samostatne).
